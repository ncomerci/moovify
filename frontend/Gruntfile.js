...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
      ...
      livereload: {
        options: {
          ...
          middleware: function (connect) {
            ...
            modRewrite([
              '!\\.html|\\.js|\\.svg|\\.css|\\.png|\\.jpg|\\.ico$ /index.html [L]',
              '.*resources/(.*)$ /$1 [L]'
            ]),
    ...
    usemin: {
      ...
      options: {
        assetsDirs: ['dist', '<%= yeoman.dist %>', '<%= yeoman.dist %>/images'],
        patterns: {
          imagesAndViews: [
    ...
    requirejs: {
      compile: {
        options: {
          mainConfigFile: '<%= yeoman.tmp %>/scripts/build.js',
  ...
  grunt.registerTask('build', 'Compiles app for production or release candidate', function () {
    grunt.task.run([
      ...
      // Creates file map from filerev result in: <<>> out: <<>> for require js :D
      'jsrev',
      // // ???
      'cdnify',
      // minify js in: <<>> out: <<>>
